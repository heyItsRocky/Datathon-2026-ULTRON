...
from catalyst_light import datastore
import json
...
def query_table(table_name, columns="*", conditions=None, limit=100, offset=0, order_by=None):
    ...
    try:
        ...
        if order_by:
            ...
            query = query.order_by(col, direction)
...
def insert_record(table_name, data):
    ...
    try:
        table = datastore.table(table_name)
        result = table.insert().set(data).execute()
        return result
    except Exception as e:
        raise Exception(f"ZCQL insert error on {table_name}: {str(e)}")
...
def update_record(table_name, data, column, value):
    ...
    try:
        table = datastore.table(table_name)
        result = table.update().set(data).where(column, "=", value).execute()
        return result
    except Exception as e:
        raise Exception(f"ZCQL update error on {table_name}: {str(e)}")
...
def delete_record(table_name, column, value):
    ...
    try:
        table = datastore.table(table_name)
        result = table.delete().where(column, "=", value).execute()
        return result
    except Exception as e:
        raise Exception(f"ZCQL delete error on {table_name}: {str(e)}")
...
def count_records(table_name, conditions=None):
    ...
    try:
        table = datastore.table(table_name)
        query = table.select("COUNT(*) as total")
        if conditions:
            for col, op, val in conditions:
                query = query.where(col, op, val)
        ...
        raise Exception(f"ZCQL count error on {table_name}: {str(e)}")
...
def get_crime_stats():
    ...
    Returns: {
        total_cases, by_type, by_district, by_status,
        by_month, conviction_rate, clearance_rate
    }
    """
    try:
        # Total cases
...
def get_trends(months=12):
    ...
    try:
        # Group by month (FIR_DATE text, so we extract first 7 chars YYYY-MM)
        results = query_table(
            TABLE_CRIMES,
...
def get_hotspots(eps=0.5, min_samples=5):
    ...
    Args:
        eps: DBSCAN epsilon parameter (distance in degrees ~50km)
        min_samples: Minimum points for cluster
    
    Returns:
        List of {cluster_id, lat, lng, count, crime_types}
    """
    try:
        from sklearn.cluster import DBSCAN
        import numpy as np
        
        # Get all cases with location data
        results = query_table(
            TABLE_CRIMES,
        ...
        for link in links:
            ...
            if fir_no and fir_no not in visited_cases:
                ...
                if case:
                    ...
                    # Find other criminals linked to this case
                    if depth > 1:
